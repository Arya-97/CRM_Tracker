...
function startGoogleAuth(){
  if(!HARDCODED_CLIENT_ID||HARDCODED_CLIENT_ID==='YOUR_CLIENT_ID_HERE'){showAuthErr('⚙️ Admin config required: Set HARDCODED_CLIENT_ID');return;}
  if(!HARDCODED_SHEET_ID||HARDCODED_SHEET_ID==='YOUR_SHEET_ID_HERE'){showAuthErr('⚙️ Admin config required: Set HARDCODED_SHEET_ID');return;}
...
async function onTokenReceived(r){
  ...
  try{
    ...
    if(!found){
      if(users.length===0){
        await sheetsAppend(SHEETS.users,[[uInfo.email,'admin','system',nowStr(),'']]);
        await loadUsers();
        currentUser={...uInfo,role:'admin',manager:''};
        bootApp();
      }else{hideLoader();showAuthErr('❌ Access Denied\n\nEmail not registered: '+uInfo.email+'\n\nRequest access or contact admin.');accessToken=null;}
    }else{
...
async function ensureSheets(){
  ...
  const toCreate=[SHEETS.main,SHEETS.audit,SHEETS.users,SHEETS.requests].filter(n=>!existing.includes(n));
  if(toCreate.length){
    await sheetsBatch(toCreate.map(t=>({addSheet:{properties:{title:t}}})));
    if(toCreate.includes(SHEETS.main))await sheetsUpdate(`${SHEETS.main}!A1`,[MAIN_HEADERS]);
    if(toCreate.includes(SHEETS.audit))await sheetsUpdate(`${SHEETS.audit}!A1`,[['timestamp','agent_email','agent_name','regno','student_name','field','old_value','new_value']]);
    if(toCreate.includes(SHEETS.users))await sheetsUpdate(`${SHEETS.users}!A1`,[['email','role','added_by','added_on','manager']]);
    if(toCreate.includes(SHEETS.requests))await sheetsUpdate(`${SHEETS.requests}!A1`,[['email','name','manager_email','status','submitted_on']]);
  }
}
...
async function loadAll(){
  ...
  try{
    ...
    if(cached&&!btn.classList.contains('force-refresh')){
      ...
      hideLoader();showToast('✅ Loaded from cache','info');
    ...
    buildFilters();applyFilters();renderAudit();renderUsersList();renderTeamMembers();renderAccessRequests();
    hideLoader();showToast('✅ Synced','ok');
  }catch(e){hideLoader();showToast('❌ '+e.message,'err');}
  btn.classList.remove('spinning');btn.classList.remove('force-refresh');
}

async function loadMainData(){const res=await sheetsGet(`${SHEETS.main}!A1:U2000`);const rows=res.values||[];if(rows.length<=1){allData=[];return;}allData=rows.slice(1).map((row,i)=>{const r={};Object.entries(COLUMNS).forEach(([k,ci])=>r[k]=(row[ci]||'').toString().trim());r._rowIndex=i+2;return r;}).filter(r=>r.regno);}
...
function onRegionChange(){const sr=getValue('f-region');const cs=document.getElementById('f-center');if(!sr){fillSelect('f-center',allCenters,'All Centers');}else{const cir=regionCenterMap[sr]?Array.from(regionCenterMap[sr]):[];fillSelect('f-center',cir,'All Centers');}applyFilters();}

let filterTimeout;
function applyFilters(){clearTimeout(filterTimeout);filterTimeout=setTimeout(()=>{const region=getValue('f-region'),center=getValue('f-center'),payment=getValue('f-payment'),form=getValue('f-form'),connected=getValue('f-connected'),dialled=getValue('f-dialled'),search=getValue('f-search').toLowerCase();filteredData=allData.filter(r=>{if(region&&r.region!==region)return false;if(center&&r.center!==center)return false;if(payment&&r.newpayment_checks!==payment)return false;if(form&&r.form_status!==form)return false;if(connected&&r.connected_status!==connected)return false;if(dialled&&r.dialled_status!==dialled)return false;if(search&&!r.regno.toLowerCase().includes(search)&&!r.student_name.toLowerCase().includes(search))return false;return true;});renderTable();updateStats();},150);}

function clearFilters(){['f-region','f-center','f-payment','f-form','f-connected','f-dialled'].forEach(id=>document.getElementById(id).value='');document.getElementById('f-search').value='';fillSelect('f-center',allCenters,'All Centers');applyFilters();}
...
// Table
function renderTable(){const tbody=document.getElementById('tbl-body');if(!filteredData.length){tbody.innerHTML=`<tr><td colspan="21"><div class="empty"><div class="empty-icon">🔍</div><div class="empty-t">No records</div></div></td></tr>`;return;}tbody.innerHTML=filteredData.map(r=>{const idx=allData.indexOf(r);const sel=selectedIdx===idx?'selected':'';return`<tr onclick="openPanel(${idx})" class="${sel}"><td class="regno-cell">${r.regno}</td><td class="name-cell">${r.student_name}</td><td>${r.center||'—'}</td><td>${bBadge(r.region,'blue')}</td><td>${payBadge(r.newpayment_checks)}</td><td>${r.fees_amt||'—'}</td><td>${r.fees_paid||'—'}</td><td>${r.emi1_paid_date||'—'}</td><td>${r.emi2_paid_date||'—'}</td><td>${r.emi3_paid_date||'—'}</td><td>${r.emi4_paid_date||'—'}</td><td>${r.total_paid_date||'—'}</td><td>${formBadge(r.form_status)}</td><td>${r.attendance_15days||'—'}</td><td>${r.last_punch_date||'—'}</td><td>${r.ptp?bBadge('✓','green'):'—'}</td><td>${connBadge(r.connected_status)}</td><td>${dialBadge(r.dialled_status)}</td><td>${dispBadge(r.yesterday_disposition)}</td><td>${dispBadge(r.todays_disposition)}</td><td><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openPanel(${idx})">Edit</button></td></tr>`;}).join('');}
...
// Save
async function saveChanges(){if(selectedIdx===null)return;const r=allData[selectedIdx];const newVals={ptp:document.getElementById('e-ptp').value.trim(),connected_status:getValue('e-connected'),dialled_status:getValue('e-dialled'),todays_disposition:getValue('e-today'),other_remarks:document.getElementById('e-remarks').value.trim()};const labels={ptp:'PTP',connected_status:'Connected Status',dialled_status:'Dialled Status',todays_disposition:"Today's Disposition",other_remarks:'Other Remarks'};const changes=Object.keys(newVals).filter(k=>(r[k]||'')!==(newVals[k]||'')).map(k=>({key:k,field:labels[k],old:r[k]||'',new:newVals[k]}));if(!changes.length){showToast('No changes','info');return;}const updated={...r};if(newVals.todays_disposition&&newVals.todays_disposition!==r.todays_disposition&&r.todays_disposition){updated.yesterday_disposition=r.todays_disposition;changes.push({key:'yesterday_disposition',field:"Yesterday's Disposition (auto)",old:r.yesterday_disposition||'',new:r.todays_disposition});}Object.assign(updated,newVals);const btn=document.getElementById('save-btn');btn.textContent='Saving…';btn.disabled=true;showLoader('Writing…');try{await writeRowToSheet(r._rowIndex,updated,changes);Object.assign(allData[selectedIdx],updated);const now=nowStr();changes.forEach(c=>auditLog.unshift({timestamp:now,agent_email:currentUser.email,agent_name:currentUser.name||currentUser.email,regno:r.regno,student_name:r.student_name,field:c.field,old_value:c.old,new_value:c.new}));document.getElementById('audit-count').textContent=auditLog.length;renderTable();renderAudit();openPanel(selectedIdx);hideLoader();showToast('✅ Saved','ok');}catch(e){hideLoader();showToast('❌ '+e.message,'err');}btn.textContent='💾 Save to Sheet';btn.disabled=false;}
...
// Access Requests
function renderAccessRequests(){const el=document.getElementById('requests-list');if(!el)return;let reqs=accessRequests;if(currentUser.role==='manager'){reqs=reqs.filter(r=>r.manager_email.toLowerCase()===currentUser.email.toLowerCase());}const pending=reqs.filter(r=>r.status==='pending');if(!pending.length){el.innerHTML='<div class="empty"><div class="empty-icon">📭</div><div class="empty-t">No pending requests</div></div>';return;}el.innerHTML=pending.map(req=>`<div class="request-card"><div class="request-header"><div class="request-info"><h4>${req.name}</h4><p>${req.email}</p></div><span class="request-status status-pending">Pending</span></div><div class="request-details"><div class="request-detail"><strong>Manager</strong><span>${req.manager_email}</span></div><div class="request-detail"><strong>Submitted</strong><span>${req.submitted_on}</span></div></div><div class="request-actions"><button class="btn btn-success btn-sm" onclick="approveRequest('${req.email}')">✓ Approve</button><button class="btn btn-danger btn-sm" onclick="declineRequest('${req.email}')">✗ Decline</button></div></div>`).join('');}

async function approveRequest(email){showLoader('Approving…');try{const req=accessRequests.find(r=>r.email.toLowerCase()===email.toLowerCase()&&r.status==='pending');if(!req){showToast('Request not found','err');hideLoader();return;}await sheetsUpdate(`${SHEETS.requests}!D${req._rowIndex}`,[ ['approved']]);await sheetsAppend(SHEETS.users,[[email,'agent',currentUser.email,nowStr(),req.manager_email]]);await loadUsers();await loadAccessRequests();renderAccessRequests();cache.clear();hideLoader();showToast('✅ Approved','ok');}catch(e){hideLoader();showToast('Error: '+e.message,'err');}}

async function declineRequest(email){if(!confirm('Decline request from '+email+'?'))return;showLoader('Declining…');try{const req=accessRequests.find(r=>r.email.toLowerCase()===email.toLowerCase()&&r.status==='pending');if(!req){showToast('Request not found','err');hideLoader();return;}await sheetsUpdate(`${SHEETS.requests}!D${req._rowIndex}`,[['declined']]);await loadAccessRequests();renderAccessRequests();hideLoader();showToast('Request declined','ok');}catch(e){hideLoader();showToast('Error: '+e.message,'err');}}
...
// Nav
function toggleSidebar(){const sidebar=document.querySelector('.sidebar');const c=sidebar.classList.toggle('collapsed');localStorage.setItem('crm_sidebar_collapsed',c?'true':'false');}
...
setInterval(()=>{if(currentUser&&document.getElementById('app').style.display==='flex'){console.log('Auto-refresh');const btn=document.getElementById('sync-btn');btn.classList.add('force-refresh');loadAll();}},5*60*1000);
